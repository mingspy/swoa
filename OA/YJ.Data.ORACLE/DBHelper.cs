    ...
    public class DBHelper : IDBHelper
    {
        ...
        public int DataTableToDB(DataTable dt)
        {
            ...
            using (OracleConnection connection = new OracleConnection(this.ConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (OracleDataAdapter adapter = new OracleDataAdapter("select * from " + dt.TableName + " where 1=0", connection))
                    {
        ...
        public string GetPaerSql(string sql, int size, int number, out long count, [Optional, DefaultParameterValue(null)] OracleParameter[] param)
        {
            long num;
        ...
        public string GetPaerSql(string table, string fileds, string where, string order, int size, int number, out long count, [Optional, DefaultParameterValue(null)] OracleParameter[] param)
        {
            long num;
            ...
            string str3 = string.Format("SELECT PagerTempTable.*,ROWNUM AS PagerAutoRowNumber FROM (SELECT {0} FROM {1} {2} ORDER BY {3}) PagerTempTable", args);
