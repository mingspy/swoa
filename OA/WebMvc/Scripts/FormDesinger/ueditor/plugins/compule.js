UE.compule = {
    ...
    getRadioOrCheckboxHtml: function ($control, type)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            if ('radio' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetRadiosByID("' + dictid + '".ToGuid(), "' + id + '", YJ.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ('checkbox' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetCheckboxsByID("' + dictid + '".ToGuid(), "' + id + '", YJ.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            if ("radio" == type)
            {
                radios += '<span>@Html.Raw(new YJ.Platform.WorkFlowForm().GetRadioFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ("checkbox" == type)
            {
                radios += '<span>@Html.Raw(new YJ.Platform.WorkFlowForm().GetCheckboxFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            $control.after(radios);
            $control.remove();
        }
    ...
    getSelectHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            var valuefiled = $control.attr("valuefiled") || "YJ.Platform.Dictionary.OptionValueField.ID";
            if ("1" == hasempty)
            {
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            radios += '@Html.Raw(new YJ.Platform.WorkFlowForm().GetOptionsFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + defaultvalue + '"))';
            radios += '</select>';
            var $radios = $(radios);
            $radios.attr("eventsid", $control.attr("eventsid"));
            $control.after($radios);
    ...
    getComboxHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            if ("1" != listmode)
            {
                ...
                radios += '@Html.Raw(BDictionary.GetOptionsByID("' + dictid + '".ToGuid(), YJ.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '"))';
                radios += '</select>';
                var $radios = $(radios);
                $radios.attr("eventsid", $control.attr("eventsid"));
                $control.after($radios);
            ...
            else
            {
                ...
                table += '>';
                table += '@Html.Raw(BDictionary.GetComboxTableHtmlByID("' + dictid + '",YJ.Platform.Dictionary.OptionValueField.ID,"' + defaultvalue + '"))';
                table += '</span>';
                var $table = $(table);
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            else
            {
                ...
                table += "1" == ismultiple ? ' more="1"' : '';
                table += '>';
                table += '@Html.Raw(new YJ.Platform.WorkFlowForm().GetComboxTableHtmlFromSql("' + dbconn + '", "' + sql.replaceAll('"', '\"') + '", "' + defaultvalue + '"))';
                table += '</span>';
                var $table = $(table);
                $table.attr("eventsid", $control.attr("eventsid"));
                $control.after($table);
        ...
        else if ("3" == datasource)//URL
        {
            if ("1" != listmode)
            {
                ...
                radios += '@Html.Raw(new YJ.Platform.WorkFlowForm().GetOptionsFromUrl("' + encodeURIComponent(url) + '", "' + defaultvalue + '"))';
                radios += '</select>';
                var $radios = $(radios);
                $radios.attr("eventsid", $control.attr("eventsid"));
                $control.after($radios);
    ...
    getOrgHtml: function ($control)
    {
        ...
        switch (org_rang)
        {
            case "0": //发起者部门
                ...
                break;
            case "1": //处理者部门
                rootid = '@YJ.Platform.Users.CurrentDeptID';
                break;
            case "2": //自定义
    ...
    getSubTableHtml_Select: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "select_dsdict":
                ...
                select += '@Html.Raw(BDictionary.GetOptionsByID("' + rootid + '".ToGuid(), YJ.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", ' + (select_ds_dict_ischild ? "true" : "false") + '))';
                break;
            case "select_dssql":
                ...
                select += '@Html.Raw(new YJ.Platform.WorkFlowForm().GetOptionsFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + dvalue + '"))';
                break;
    ...
    getSubTableHtml_Checkbox: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "checkbox_dsdict":
                ...
                checkbox = '<span>@Html.Raw(BDictionary.GetCheckboxsByID("' + rootid + '".ToGuid(), "' + name + '", YJ.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'"))</span>';
                break;
            case "checkbox_dssql":
                var conn = editmode.checkbox_ds_dbconn;
                var sql = editmode.checkbox_ds_sql;
                checkbox = '@Html.Raw(new YJ.Platform.WorkFlowForm().GetCheckboxFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'"))';
                break;
    ...
    getSubTableHtml_Radio: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "radio_dsdict":
                ...
                radio = '<span>@Html.Raw(BDictionary.GetRadiosByID("' + rootid + '".ToGuid(), "' + name + '", YJ.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\' colname=\'' + colnumJSON.name + '\'"))</span>';
                break;
            case "radio_dssql":
                var conn = editmode.radio_ds_dbconn;
                var sql = editmode.radio_ds_sql;
                radio = '@Html.Raw(new YJ.Platform.WorkFlowForm().GetRadioFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\'" colname=\'' + colnumJSON.name + '\'"))';
                break;
            case "radio_dsstring":
    ...
    getSubTableHtml: function ($control)
    {
        ...
        if ("1" != subtableJSON.editmodel)
        {
            ...
            for (var i = 0; i < subtableJSON.colnums.length; i++)
            {
                ...
                edittds += '<td  data-title="' + (colnumJSON.showname || colnumJSON.name) +'"  colname="' + colnumJSON.name + '" iscount="' + colnumJSON.issum + '" style="text-align:' + align + ';padding-right:3px;">';
                if (index == 0)
                {
        ...
        else //弹出方式
        {
            ...
            for (var i = 0; i < subtableJSON.colnums.length; i++)
            {
                ...
                html1 += '<th data-title="' + (colnumJSON.showname||colnumJSON.name)+'" style="text-align:left;">';
            ...
            html1 += '<tbody>';
            html1 += '@{';
            html1 += 'System.Data.DataTable Dt_' + id + ' = new YJ.Platform.DBConnection().GetDataTable(DBConnID, "' + subtableJSON.secondtable + '","' + subtableJSON.secondtablerelationfield + '", InstanceID, "' + sortstring + '");';
            html1 += 'YJ.Platform.WorkFlowForm bWorkFlowForm_' + id + ' = new YJ.Platform.WorkFlowForm();';
            ...
            for (var i = 0; i < subtableJSON.colnums.length; i++)
            {
                ...
                html1 += '<td data-title="' + (colnumJSON.showname || colnumJSON.name) +'" >';
