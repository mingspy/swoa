...
namespace YJ.Platform
{
    ...
    {
        private YJ.Data.Interface.IWorkFlowDelegation dataWorkFlowDelegation;
        private static string cacheKey = YJ.Utility.Keys.CacheKeys.WorkFlowDelegation.ToString();
        ...
        /// <returns></returns>
        public List<YJ.Data.Model.WorkFlowDelegation> GetPagerData(out string pager, string query = "", string userID = "", string startTime = "", string endTime = "")
        {
        ...
        /// <param name="endTime"></param>
        /// <returns></returns>
        public List<YJ.Data.Model.WorkFlowDelegation> GetPagerData(out long count, int pageSize, int pageNumber, string userID = "", string startTime = "", string endTime = "", string order = "")
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<YJ.Data.Model.WorkFlowDelegation> GetNoExpiredListFromCache()
        {
            object obj = YJ.Cache.IO.Opation.Get(cacheKey);
            if (obj != null && obj is List<YJ.Data.Model.WorkFlowDelegation>)
            {
                return (List<YJ.Data.Model.WorkFlowDelegation>)obj;
            }
            else
            {
                var list = GetNoExpiredList();
        ...
        public Guid GetFlowDelegationByUserID(Guid flowID, Guid userID)
        {
            ...
            var userList = list.Where(p => p.UserID == userID && (!p.FlowID.HasValue || p.FlowID.Value == Guid.Empty || p.FlowID.Value == flowID) && p.StartTime <= YJ.Utility.DateTimeNew.Now && p.EndTime >= YJ.Utility.DateTimeNew.Now);
        ...
        private Guid getFlowDelegationByUserID1(Guid flowID, Guid userID, List<YJ.Data.Model.WorkFlowDelegation> list)
        {
