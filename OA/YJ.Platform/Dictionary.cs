...
namespace YJ.Platform
{
    ...
    {
        private YJ.Data.Interface.IDictionary dataDictionary;
        private static string cacheKey = YJ.Utility.Keys.CacheKeys.Dictionary.ToString();
        public Dictionary()
        {
        ...
        /// 查询所有记录
        /// </summary>
        public List<YJ.Data.Model.Dictionary> GetAll(bool fromCache = false)
        {
            if (!fromCache)
            {
            ...
            {
                List<YJ.Data.Model.Dictionary> list = new List<Data.Model.Dictionary>();
                object obj = YJ.Cache.IO.Opation.Get(cacheKey);
                if (obj != null && obj is List<YJ.Data.Model.Dictionary>)
                {
                    list = (List<YJ.Data.Model.Dictionary>)obj;
                }
                if (list.Count > 0)
                {
                    return list;
        ...
        /// 查询单条记录
        /// </summary>
        public YJ.Data.Model.Dictionary Get(Guid id, bool fromCache = false)
        {
            if (fromCache)
            {
                var dict = GetAll(true).Find(p => p.ID == id);
                return dict == null ? dataDictionary.Get(id) : dict;
            }
            else
            { 
        ...
        /// 查询下级记录
        /// </summary>
        public List<YJ.Data.Model.Dictionary> GetChilds(Guid id, bool fromCache = false)
        {
        ...
        /// 查询下级记录
        /// </summary>
        public List<YJ.Data.Model.Dictionary> GetChilds(string code, bool fromCache = false)
        {
            return code.IsNullOrEmpty() ? new List<YJ.Data.Model.Dictionary>() :
                fromCache ? getChildsByCodeFromCache(code) :
                dataDictionary.GetChilds(code.Trim());
        }

        private List<YJ.Data.Model.Dictionary> getChildsByCodeFromCache(string code)
        {
            var list = GetAll(true);
            var dict = list.Find(p => string.Compare(p.Code, code, true) == 0);
            return dict == null ? new List<YJ.Data.Model.Dictionary>() : list.FindAll(p => p.ParentID == dict.ID).OrderBy(p=>p.Sort).ToList();
        }

        private List<YJ.Data.Model.Dictionary> getChildsByIDFromCache(Guid id)
        {
            var list = GetAll(true);
        ...
        /// <returns></returns>
        public List<YJ.Data.Model.Dictionary> GetAllParents(Guid id, bool isMe = true)
        {
            ...
            addParent(list, dict.ParentID);
            return list;
        }

        private void addParent(List<YJ.Data.Model.Dictionary> list, Guid id)
        {
        ...
        /// <returns></returns>
        public List<YJ.Data.Model.Dictionary> GetAllChilds(string code, bool fromCache)
        {
            if (code.IsNullOrEmpty()) return new List<YJ.Data.Model.Dictionary>();
            var dict = GetByCode(code, fromCache);
            if (dict == null) return new List<YJ.Data.Model.Dictionary>();
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public List<YJ.Data.Model.Dictionary> GetAllChilds(Guid id, bool fromCache=false)
        {
            List<YJ.Data.Model.Dictionary> list = new List<YJ.Data.Model.Dictionary>();
        ...
        private void addChilds(List<YJ.Data.Model.Dictionary> list, Guid id, bool fromCache=false)
        {
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public YJ.Data.Model.Dictionary GetByCode(string code, bool fromCache=false)
        {
        ...
        /// <param name="isChild">是否显示所有下级</param>
        /// <returns></returns>
        public string GetOptionsByID(Guid id, OptionValueField valueField = OptionValueField.Value, string value = "", bool isChild = true)
        {
            ...
            var childs = isChild ? GetAllChilds(id, true) : GetChilds(id, true);
            ...
            foreach (var child in childs)
            {
                ...
                options.AppendFormat("<option value=\"{0}\"{1}>{2}{3}</option>", value1, value1 == value ? " selected=\"selected\"" : "", space.ToString(), child.Title);
            }
        ...
        /// <returns></returns>
        private int getParentCount(List<YJ.Data.Model.Dictionary> dictList, YJ.Data.Model.Dictionary dict)
        {
            int parent = 0;
            YJ.Data.Model.Dictionary parentDict = dictList.Find(p => p.ID == dict.ParentID);
        ...
        /// <param name="valueField"></param>
        /// <param name="value"></param>
        /// <param name="isChild">是否加载所有下级</param>
        /// <returns></returns>
        ...
        private string getRadios(List<YJ.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getCheckboxs(List<YJ.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getOptionsValue(OptionValueField valueField, YJ.Data.Model.Dictionary dict)
        {
        ...
        {
            YJ.Cache.IO.Opation.Set(cacheKey, GetAll());
        }
        ...
        /// <returns></returns>
        public string GetComboxTableHtmlByID(string id, YJ.Platform.Dictionary.OptionValueField valueField, string defaultValue)
        {
            if (!id.IsGuid())
            {
                return "";
