...
namespace YJ.Utility
{
  public class Tools
  {
    [Obsolete("请用YJ.Utility.DateTimeNew.Now", true)]
    ...
    public static MemoryStream GetValidateImg(out string code, string bgImg = "/Images/vcodebg.png")
    {
      ...
      Font fontbg = new Font("Arial", 16f, FontStyle.Italic);
      ...
      graphics.DrawString(code, fontbg, Brushes.White, 0.0f, 1f);
      graphics.DrawString(code, font1, Brushes.Green, 0.0f, 1f);//字颜色
      //画图片的背景噪音线 
    ...
    public static int GetPageSize()
    {
      ...
      if (!str.IsInt())
      {
        HttpCookie cookie = HttpContext.Current.Request.Cookies["roadflowpagesize"];// ysj标记
        if (cookie != null)
          str = cookie.Value;
      }
      int test;
      ...
      if (num <= 0)
        num = 15;
      HttpContext.Current.Response.Cookies.Add(new HttpCookie("roadflowpagesize")//ysj标记
      {
        Value = num.ToString(),
    ...
    public static string GetDateTimeDifferString(DateTime dt1, DateTime dt2)
    {
      ...
      return "";
        ...
        public static bool  PasswordStrength(string password)
        {
            ...
            {
                if (c >= '0' && c <= '9') iNum++;//数字
                else if (c >= 'a' && c <= 'z') iLtt++;//字母小写
                else if (c >= 'A' && c <= 'Z') iLtt++;//字母大写
                else iSym++;//符号
            }
            ...
            //if (iLtt == 0) return false; //数字和符号构成的密码
           //if (iSym == 0) return false; //数字和字母构成的密码
           //if (iNum == 0) return false;//字母和符号构成的密码
            return true; //{ RSL = true, MSG = "密码符合" };
        }
