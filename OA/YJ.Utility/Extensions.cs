...
// Type: MyExtensions
// Assembly: YJ.Utility, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: E4D91F62-39BF-4125-A013-6EDB7CF1B4EC
// Assembly location: C:\inetpub\wwwroot\RoadFlowMvc\bin\YJ.Utility.dll
...
public static class MyExtensions
{
    ...
    public class MyUnit
    {
        public MyUnit(float num, string un)
        {
            this.Num = num;
            this.Unit = un;
        }
    ...
    private static Dictionary<char, float> numDict = initNumDict();
    private static Dictionary<char, int> mydateDict = initDateDict();
    private static HashSet<char> chineseNumSet = new HashSet<char>("零一二三四五六七八九十百千万");

    private static Dictionary<char, int> initDateDict()
    {
    ...
    public static float ChineseToNum(this string str){
        ...
        if (m > 9)
        {
            float left = 1f;
            if (idx > 0)
                left = str.Substring(0, idx).ChineseToNum();
            ret = left* m + str.Substring(idx + 1).ChineseToNum();
        }
        else {
    ...
    private static Dictionary<char, float> initNumDict()
    {
        Dictionary<char, float> ret = new Dictionary<char, float>();
        int i = 0;
        foreach (char s in "0123456789零一二三四五六七八九")
        {
            ret[s] = i++;
            if (i == 10)
                i = 0;
    ...
    public static MyUnit ToNumUnit(this string str)
    {
        MyUnit ret = new MyUnit(0, "");
        if (string.IsNullOrEmpty(str))
        {
            return ret;
        }
        ...
        foreach( char c in str)
        {
            if (numDict.ContainsKey(c) || c.Equals('.'))
            {
                j += 1;
            }
            else
        ...
        ret.Unit = str.Substring(j);
        str = str.Substring(0, j);
        i = j = 0;
        if (is_chinese)
        {
           ret.Num = str.ChineseToNum();
        }
        else
        {
            decimal res;
    ...
    public static DateTime ToExpireDate(this string str, DateTime start)
    {
        if(string.IsNullOrEmpty(str) )
            return DateTime.MaxValue;
        str = str.RemoveSpace();
        if (string.IsNullOrEmpty(str) || str.Equals( "/") || str.Contains("长期"))
        {
            return DateTime.MaxValue;
        }
        int i = -1;
        ...
        if (i >= 0)
        {
            if(str.Length >= 4)
            {
                var r = str.Substring(i).ToDateTimeOrNull();
                if (r != null)
                {
                    return (DateTime)r;
                }
            ...
            foreach (var c in mydateDict)
            {
                int t = str.LastIndexOf(c.Key);
                if (t > j)
                {
                    j = t;
                }
        ...
        if (j > i && i >= 0)
        {
            str = str.Substring(i, j - i + 1);
           
        }

        if (str.Length >= 4)
        {
            var ret = str.ToDateTimeOrNull();
            if (ret != null)
            {
                return (DateTime)ret;
            }
        ...
        var ret2 = start;
        int sum = 0;
        foreach( var u in "年月日天")
        {
            i = str.IndexOf(u);
            if (i >= 0)
            {
                sum += (int)str.Substring(0,i).ToNumUnit().Num * mydateDict[u];
                str = str.Substring(i + 1);
            }
        }
        return ret2.AddDays(sum);
    }
